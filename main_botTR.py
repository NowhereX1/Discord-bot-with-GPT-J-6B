...
async def on_message(message):
    ...
    if message.content.startswith("!"):
        ...
        try:
            ...
            tr = GoogleTranslator(source="auto", target="tr").translate(response)
            await message.channel.send(tr,reference=message)
            ...
            if url[0:26] == "https://cdn.discordapp.com":   
                ...
                text = tess.image_to_string(img,lang="tur")
                responseText = gpt_api.query(text,parameters)
                translateText = GoogleTranslator(source="auto", target="tr").translate(responseText)
                await message.channel.send(translateText,reference=message)
                os.remove("image.png")
